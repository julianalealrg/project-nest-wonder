...
function mapExtractedToOS(d: any, file: File): OSForm {
  ...
  if (d?.projetista) {
    const match = PROJETISTAS.find((p) => p.toLowerCase() === d.projetista.toLowerCase());
    if (match) {
      os.projetista = match;
    } else {
      os.projetista = "outro";
      os.projetista_outro = d.projetista;
    }
  }

  if (d?.pecas && Array.isArray(d.pecas) && d.pecas.length > 0) {
    os.pecas = d.pecas.map((p: any) => ({
      item: p.item || "",
...
export function NovaOSDialog({ open, onOpenChange, onSuccess }: NovaOSDialogProps) {
  ...
  useEffect(() => {
    if (!open) return;
    async function load() {
      const { data } = await supabase.from("system_lists").select("tipo, valor");
      if (data) {
  ...
  function updatePeca(osIdx: number, pecaIdx: number, partial: Partial<PecaForm>) {
    setOsList((prev) =>
      prev.map((os, i) =>
        i === osIdx
          ? { ...os, pecas: os.pecas.map((p, j) => (j === pecaIdx ? { ...p, ...partial } : p)) }
          : os
      )
    );
  }
  ...
  function addPeca(osIdx: number) {
    setOsList((prev) =>
      prev.map((os, i) => (i === osIdx ? { ...os, pecas: [...os.pecas, emptyPeca()] } : os))
    );
  }
  ...
  const handlePdfUpload = useCallback(async (files: FileList | File[]) => {
    ...
    if (pdfFiles.length === 0) {
      toast({ title: "Selecione arquivos PDF", variant: "destructive" });
      return;
    }
    ...
    const extractedOSList: OSForm[] = [];
    let firstClientData: any = null;
    let errors: string[] = [];

    for (let i = 0; i < pdfFiles.length; i++) {
      setParseProgress({ current: i + 1, total: pdfFiles.length });
      try {
        const data = await parseSinglePdf(pdfFiles[i]);
        if (!data) {
          errors.push(pdfFiles[i].name);
          continue;
        }
        if (!firstClientData) firstClientData = data;
        extractedOSList.push(mapExtractedToOS(data, pdfFiles[i]));
      } catch (err: any) {
        console.error(`Error parsing ${pdfFiles[i].name}:`, err);
        errors.push(pdfFiles[i].name);
      }
    }
    ...
    if (firstClientData) {
      ...
      if (firstClientData.supervisor) {
        const match = SUPERVISORES.find(
          (s) => s.toLowerCase() === firstClientData.supervisor.toLowerCase()
        );
        if (match) {
          newCliente.supervisor = match;
        } else {
          newCliente.supervisor = "outro";
          newCliente.supervisor_outro = firstClientData.supervisor;
        }
      }
      setCliente(newCliente);
    }

    if (extractedOSList.length > 0) {
      setOsList(extractedOSList);
      setParseSuccess(true);
      const msg = errors.length > 0
        ? `${extractedOSList.length} OS extraídas. ${errors.length} PDF(s) com erro: ${errors.join(", ")}`
        : `${extractedOSList.length} OS extraídas dos PDFs. Confira antes de salvar.`;
      toast({ title: msg });
    } else {
      toast({ title: "Não foi possível extrair dados dos PDFs", variant: "destructive" });
    }
  ...
  const handleFileInput = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) handlePdfUpload(e.target.files);
    },
    [handlePdfUpload]
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      for (const os of osList) {
        ...
        // Upload PDF if from import
        if (os._pdfFile) {
          const pdfUrl = await uploadPdfToStorage(newOS.id, os._pdfFile);
          if (pdfUrl) {
            await supabase.from("ordens_servico").update({ pdf_url: pdfUrl }).eq("id", newOS.id);
          }
        }
  ...
  // Render pieces table for an OS
  function renderPecasTable(os: OSForm, osIdx: number) {
    return (
      <div className="space-y-2">
                ...
                <tr key={pecaIdx} className="border-b last:border-0">
                  ...
                  <td className="py-1 px-1">
                    <Input
                      ...
                      onChange={(e) => updatePeca(osIdx, pecaIdx, { quantidade: Number(e.target.value) || 1 })}
                      className="h-8 text-xs"
                    />
                  </td>
                  <td className="py-1 px-1">
                    <Input
                      ...
                      onChange={(e) =>
                        updatePeca(osIdx, pecaIdx, { comprimento: e.target.value === "" ? "" : Number(e.target.value) })
                      }
                      placeholder="1.9400"
                      className="h-8 text-xs"
                    />
                  ...
                  <td className="py-1 px-1">
                    <Input
                      ...
                      value={peca.largura}
                      onChange={(e) =>
                        updatePeca(osIdx, pecaIdx, { largura: e.target.value === "" ? "" : Number(e.target.value) })
                      }
                      placeholder="0.3700"
                      className="h-8 text-xs"
                    />
                  ...
                  <td className="py-1 px-1 text-center">
                    <Checkbox checked={peca.precisa_45} onCheckedChange={(v) => updatePeca(osIdx, pecaIdx, { precisa_45: !!v })} />
                  </td>
                  <td className="py-1 px-1 text-center">
                    <Checkbox checked={peca.precisa_poliborda} onCheckedChange={(v) => updatePeca(osIdx, pecaIdx, { precisa_poliborda: !!v })} />
                  </td>
                  <td className="py-1 px-1 text-center">
                    <Checkbox checked={peca.precisa_usinagem} onCheckedChange={(v) => updatePeca(osIdx, pecaIdx, { precisa_usinagem: !!v })} />
                  </td>
                  <td className="py-1 px-1">
                    {os.pecas.length > 1 && (
                      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => removePeca(osIdx, pecaIdx)}>
        ...
        {(() => {
          ...
          return totalM2 > 0 ? (
            <div className="text-xs font-medium text-right text-foreground">M² total: {totalM2.toFixed(4)}</div>
          ) : null;
        })()}
        <Button variant="outline" size="sm" type="button" onClick={() => addPeca(osIdx)} className="text-xs">
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] p-0 gap-0">
                ...
                {parsing ? (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Extraindo dados do PDF {parseProgress.current}/{parseProgress.total}...
                    </p>
                    <p className="text-xs text-muted-foreground">Isso pode levar alguns segundos por PDF</p>
                  </div>
                ) : parseSuccess ? (
                  <div className="flex flex-col items-center gap-3">
                    <CheckCircle2 className="h-8 w-8 text-green-600" />
                    <p className="text-sm font-medium text-green-700">
                      {osList.length} OS extraída{osList.length > 1 ? "s" : ""} dos PDFs. Confira antes de salvar.
                    </p>
                    <div className="flex flex-wrap gap-1 justify-center">
                      {osList.map((os, i) => (
                        <Badge key={i} variant="secondary" className="text-xs">
                          {os._pdfFile?.name || `OS ${i + 1}`}
                        </Badge>
                      ))}
                    </div>
                    <Button
                  ...
                  <label className="flex flex-col items-center gap-3 cursor-pointer">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Arraste os PDFs das OS ou clique para selecionar
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Aceita múltiplos PDFs — a IA extrai os dados de cada um
                    </p>
                    <input
                      type="file"
                      accept=".pdf"
              ...
              <section key={osIdx} className="space-y-3 border rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-foreground">OS {osIdx + 1}</h3>
                    {os._pdfFile && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input value={os.material} onChange={(e) => updateOS(osIdx, { material: e.target.value })} placeholder="Ex: Quartzito Taj Mahal" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs">Projetista</Label>
                    <Select value={os.projetista} onValueChange={(v) => updateOS(osIdx, { projetista: v })}>
                      <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                      <SelectContent>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t">
          ...
          <Button onClick={handleSave} disabled={saving}>
            ...
            {mode === "pdf" && parseSuccess
              ? `Salvar ${osList.length > 1 ? `todas ${osList.length} OS` : "OS"}`
