    ...
    <div className="space-y-4">
            ...
            <TableBody>
              {profiles.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge variant={p.perfil === "admin" ? "default" : "secondary"} className="text-xs">
                      {p.perfil}
                    </Badge>
                  </TableCell>
      ...
      {editProfile && (
        <EditUserDialog
          ...
          onSaved={() => { setEditProfile(null); queryClient.invalidateQueries({ queryKey: ["admin-profiles"] }); }}
        />
      )}
      ...
      {novoOpen && (
        <NovoUserDialog
          ...
          onSaved={() => { setNovoOpen(false); queryClient.invalidateQueries({ queryKey: ["admin-profiles"] }); }}
        />
      )}
    </div>
  );
...
function EditUserDialog({ profile, onClose, onSaved }: { profile: Profile; onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSave() {
    ...
    if (error) {
      ...
      // Update user_roles if perfil changed
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          ...
          <div><Label>Função</Label><Input value={funcao} onChange={(e) => setFuncao(e.target.value)} /></div>
          <div>
            <Label>Perfil</Label>
...
function NovoUserDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSave() {
    ...
    if (data.user) {
      await supabase.from("profiles").update({ base, funcao: funcao || null, perfil: perfil as any }).eq("user_id", data.user.id);
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          ...
          <Button onClick={handleSave} disabled={saving || !email || !nome} className="w-full">
            {saving && <Loader2 className="h-4 w-4 animate-spin mr-1" />}Criar Usuário
          </Button>
        </div>
      </DialogContent>
    </Dialog>
